...
4. OBSにキー入力状況を表示したい場合は、同梱のHTMLファイル(htmlフォルダ内)をOBSにドラッグ&ドロップする。
   (URLで読み込むこともできます。後述の「URLでの読み込み」を参照)
...
## URLでの読み込み
アプリはWebSocketと同じポートでhtmlフォルダ内のファイルをHTTP配信しています。
OBSのブラウザソースで「ローカルファイル」のチェックを外し、
`http://<アプリを起動しているPCのIPアドレス>:<ポート>/iidx_1p.html` のようにURLを指定して読み込めます。
この場合、WebSocketの接続先はURLのホスト・ポートから自動で決まるため、`property.css`の`--host`を編集する必要はありません。
配信PCとゲームPCを分けた2台構成でも、ファイル共有を使わずに読み込めます。

`http://<IPアドレス>:<ポート>/` を開くと、各ページへのリンク・推奨サイズ・コピー用URLの一覧ページが表示されます。
アプリのメイン画面に表示されるURLや```ページ一覧を開く```ボタンからも開けます。
別PCから接続できない場合は、アプリを起動しているPCのWindowsファイアウォールで該当ポートの受信を許可してください。
